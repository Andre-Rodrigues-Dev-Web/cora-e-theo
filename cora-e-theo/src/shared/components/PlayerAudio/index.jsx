...
import { MdPlayArrow, MdPause, MdStop, MdRepeat, MdSkipNext, MdSkipPrevious } from 'react-icons/md';
...
const PlayerAudio = () => {
    ...
    const tracks = [
        { title: 'Pet de Estimação', author: 'Cora e Theo', src: petEstimacao, cover: capaCd },
        { title: 'With or Without', author: 'Cora e Theo', src: withOrWithout, cover: capaCd },
    ];
        ...
        <ContainerPlayerAudio>
            <Carousel>
                {tracks.map((track, index) => (
                    <Track key={index}>
                        <img src={track.cover} alt={`Capa do álbum ${track.title}`} />
                        <div className="text">
                            <h3>{track.title}</h3>
                            <p>{track.author}</p>
                            <p>Tempo da música: {currentTrack === index && isPlaying ? duration : '00:00'}</p>
                        </div>
                        {currentTrack === index && isPlaying ? (
                            <MdPause onClick={handlePause} aria-label="Pausar música" />
                        ) : (
                            <MdPlayArrow onClick={() => handlePlay(index)} aria-label="Tocar música" />
                        )}
                    </Track>
                ))}
            </Carousel>
